...
async def addchannel(client, message):
    ...
    try:
        ...
        print(e)
        await message.reply_text(
            f"<b>🛑 Flood Wait Error 🛑 \n User {user.first_name} couldn't join your group due to heavy join requests for userbot! Make sure user is not banned in group</b>",
        )
        return
    await message.reply_text(
...
async def rem(USER, message):
    try:
        ...
        await message.reply_text(
            f"<b>User couldn't leave your group! May be floodwaits</b>",
        )
        return
...
@Client.on_message(
    filters.command(["mailjoinchannel", "mailjoinc"]) & ~filters.private & ~filters.bot
)
@authorized_users_only
@errors
